...
#include "base_lidar/srv/control.hpp"
#include <sys/time.h>
...
bool control_motor(const std::shared_ptr<base_lidar::srv::Control::Request> req, std::shared_ptr<base_lidar::srv::Control::Response> res)
{
	std::string cmd;
...
int main(int argc, char *argv[])
{
	...
	rclcpp::Service<base_lidar::srv::Control>::SharedPtr control_srv[MAX_LIDARS * 2];
	...
	for (unsigned int i = 0; i < argdata.connectargs.size(); i++)
	{
		if (argdata.output_scan)
		{
			...
			control_srv[i] = node->create_service<base_lidar::srv::Control>(serviceName, control_motor);
		}
		if (argdata.output_cloud2)
		{
